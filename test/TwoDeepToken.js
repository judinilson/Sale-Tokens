...
contract("TwoDeepToken", function (accounts) {
  ...
  it("initialize the contract with the correct values", function () {
    return TwoDeepToken.deployed()
      ...
      .then(function (name) {
        assert.equal(name, "Two Deep Token", "has the correct name");
        return tokenInstance.symbol();
      })
      .then(function (symbol) {
        assert.equal(symbol, "TDT", "has the correct symbol");
        return tokenInstance.standard();
      })
      .then(function (standard) {
        assert.equal(standard, "2Deep Token v1.0", "has the correct symbol");
      });
  });
    ...
    return TwoDeepToken.deployed()
      ...
      .then(function (adminBalance) {
        assert.equal(
          ...
          1000000,
          "it allocates inital supply to the admin account "
        );
      });
  });
  ...
  it("transfers token ownership", function () {
    return TwoDeepToken.deployed()
      ...
      .then(function (receipt) {
        //receipt listen to transfer event
  ...
  it("approves tokens for delegate transfer", function () {
    return TwoDeepToken.deployed()
      .then(function (instance) {
        tokenInstance = instance;
        return tokenInstance.approve.call(accounts[1], 100); //this only call without modifying any data just checking
      })
      .then(function (success) {
        assert.equal(success, true, "it returns true");
        return tokenInstance
          .approve(accounts[1], 100, {
            from: accounts[0],
          }) // this one in other side call approve function and approve the value allowed
          .then(function (receipt) {
            assert.equal(receipt.logs.length, 1, "triggers one event");
            assert.equal(
              receipt.logs[0].event,
  ...
  it("handles delegated token transfers", function () {
    return TwoDeepToken.deployed()
      .then(function (instance) {
        ...
        spendingAccount = accounts[4]; //this the one that will call the function
      ...
      .catch(function (error) {
        ...
        return tokenInstance.transferFrom.call(fromAccount, toAccount, 10, {
          //call to check if everything is success
          from: spendingAccount,
        });
      })
      .then(function (success) {
